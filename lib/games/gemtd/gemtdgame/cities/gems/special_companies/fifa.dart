...
class FIFA extends GemComponent {
  FIFA({Vector2? position}) : super(position: position);

  @override
  GemAttributes settings = FIFASettings();
...
class FIFASettings extends GemAttributes {
  @override
  List<String> names = ["FIFA"];
  ...
  Set<Ability> abilities(int level, GemComponent caster) => {
        GreenIsGold(caster: caster, level: level),
      };
}
