...
class FoxNews extends GemComponent {
  FoxNews({Vector2? position}) : super(position: position);

  @override
  GemAttributes settings = FoxNewsSettings();
...
class FoxNewsSettings extends GemAttributes {
  @override
  List<String> names = ["Fox News"];
  ...
  Set<Ability> abilities(int level, GemComponent caster) => {
        EchoChamber(caster: caster, level: level),
      };
}
