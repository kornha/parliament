...
class NBA extends GemComponent {
  NBA({Vector2? position}) : super(position: position);

  @override
  GemAttributes settings = NBASettings();
...
class NBASettings extends GemAttributes {
  @override
  List<String> names = ["NBA"];
  ...
  Set<Ability> abilities(int level, GemComponent caster) => {
        Telescope(caster: caster, level: level),
      };
}
