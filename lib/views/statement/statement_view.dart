import 'package:flutter/material.dart';
import 'package:hooks_riverpod/hooks_riverpod.dart';
import 'package:political_think/common/components/icon_grid.dart';
import 'package:political_think/common/components/loading.dart';
import 'package:political_think/common/components/zerror.dart';
import 'package:political_think/common/constants.dart';
import 'package:political_think/common/extensions.dart';
import 'package:political_think/common/models/platform.dart';
import 'package:political_think/common/models/statement.dart';
import 'package:political_think/common/services/database.dart';
import 'package:political_think/common/services/zprovider.dart';
import 'package:political_think/views/confidence/confidence_widget.dart';
import 'package:political_think/views/bias/political_position_widget.dart';
import 'package:political_think/views/entity/entity_list.dart';

class StatementView extends ConsumerStatefulWidget {
  final String stid;

  const StatementView({
    super.key,
    required this.stid,
  });

  @override
  ConsumerState<ConsumerStatefulWidget> createState() => _StatementViewState();
}

class _StatementViewState extends ConsumerState<StatementView> {
  List<Platform>? platforms;

  // The tug-of-war line: a thin gradient rule between the pro camp (left,
  // green) and the against camp (right, red). The color split point IS the
  // pro/against ratio — consensus at a glance, no labels.
  Widget _tugOfWarLine(BuildContext context, int pro, int against) {
    final total = pro + against;
    if (total == 0) return const SizedBox.shrink();
    final ratio = pro / total;
    // Soft blend around the split point, clamped away from the edges so a
    // one-sided debate still shows a sliver of the other color's absence.
    final lo = (ratio - 0.08).clamp(0.0, 1.0);
    final hi = (ratio + 0.08).clamp(0.0, 1.0);
    return Container(
      height: 3,
      margin: const EdgeInsets.symmetric(horizontal: Margins.half),
      decoration: BoxDecoration(
        borderRadius: BRadius.circular,
        gradient: LinearGradient(
          colors: const [
            Palette.green,
            Palette.green,
            Palette.red,
            Palette.red,
          ],
          stops: [0.0, lo, hi, 1.0],
        ),
      ),
    );
  }

  @override
  Widget build(BuildContext context) {
    var statementRef = ref.watch(statementProvider(widget.stid));
    var statement = statementRef.value;

    var proListRef = ref.watch(entitiesFromPostsProvider(statement?.pro ?? []));
    var againstListRef =
        ref.watch(entitiesFromPostsProvider(statement?.against ?? []));

    var proList = proListRef.value ?? [];
    var againstList = againstListRef.value ?? [];

    List<String> plids = proList
        .where((e) => e.plid != null && e.photoURL == null)
        .map((e) => e.plid!)
        .toList();
    plids.addAll(againstList
        .where((e) => e.plid != null && e.photoURL == null)
        .map((e) => e.plid!)
        .toList());

    // TODO: hack to get the platform URL
    // note that using a provider here causes a loop
    if (plids.isNotEmpty && platforms == null) {
      Database.instance().getPlatforms(plids).then((value) {
        setState(() {
          platforms = value;
        });
      });
    }

    return statementRef.isLoading
        ? const Loading(type: LoadingType.standard)
        : statement == null
            ? const ZError(type: ErrorType.standard)
            : Column(
                mainAxisSize: MainAxisSize.min,
                children: [
                  context.sh,
                  Row(
                    children: [
                      Expanded(
                        flex: 5, // need as spacer causes issue with text
                        child: Text(
                          statement.value,
                          style: context.l,
                        ),
                      ),
                      const Spacer(),
                      // Unmeasured values render dimmed rather than implying
                      // a computed 0.50 / neutral lean.
                      Visibility(
                        visible: statement.type == StatementType.claim,
                        child: Opacity(
                          opacity: statement.confidence == null ? 0.45 : 1.0,
                          child: ConfidenceWidget(
                            confidence: statement.confidence,
                            stid: statement.stid,
                            enabled: ref.isAdmin,
                          ),
                        ),
                      ),
                      Visibility(
                        visible: statement.type == StatementType.opinion,
                        child: Opacity(
                          opacity: statement.bias == null ? 0.45 : 1.0,
                          child: PoliticalPositionWidget(
                            position: statement.bias,
                            stid: statement.stid,
                            enabled: ref.isAdmin,
                          ),
                        ),
                      )
                    ],
                  ),
                  context.sh,
                  // Pro camp left, against camp right, and a tug-of-war line
                  // between them: the green/red split IS the pro/against
                  // ratio — no labels needed.
                  Row(
                    crossAxisAlignment: CrossAxisAlignment.center,
                    children: [
                      if (proList.isNotEmpty) IconGrid(entities: proList),
                      Expanded(
                        child: _tugOfWarLine(
                          context,
                          statement.pro.length,
                          statement.against.length,
                        ),
                      ),
                      if (againstList.isNotEmpty)
                        IconGrid(entities: againstList),
                      context.sh,
                    ],
                  ),
                ],
              );
  }
}
